 ...
 * Finished
 * 
 * input data 1 2 3 4 5 6 7 8 9 0 2 4 1 4.5 2.2 1.1 4.3 5.2
 * 
 * @author Qiufan
 *
 */

public class Question07_06_Algebra_MultiplyTwoMatrices {
	...
	public static void printResult(double[][] matrix1, double[][] matrix2, double[][] resultingMatrix) {
		...
		for (int i = 0; i < result.length; i++) {
			for (int j = 0; j < result[0].length; j++) {
				for (int k = 0; k < result.length; k++) {
					result[i][j] += (double) (Math.round(a[i][k] * b[k][j] * 10)) / 10;
				}
			}
		}
		return result;
